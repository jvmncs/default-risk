...
def test_app():
    ix = 12
    exp_app_ix = torch.arange(ix * na, (ix + 1) * na, dtype=torch.float).view(1, na)
    assert (app_d[ix] == exp_app_ix).all()

# Bureau
def test_bureau():
    ix = 3
    exp_bureau_ix = torch.cat([nb * a * i + torch.arange(nb * ix, nb * (ix + 1), dtype=torch.float).view(1, -1) for i in range(b)])
...
def test_burbal():
    ix = 5
    exp_burbal_j = [torch.cat([nbb * (b * a) * i + torch.arange(nbb * jx, nbb * (jx + 1), dtype=torch.float).view(1, -1) for i in range(bb)]).unsqueeze(1) for jx in burbal_map_mock[ix]]
...
def test_prvapp():
    ix = 27
    exp_prvapp_ix = torch.cat([np * a * i + torch.arange(np * ix, np * (ix + 1), dtype=torch.float).view(1, -1) for i in range(p)])
...
def test_pos():
    ix = 13
    exp_pos_j = [torch.cat([npc * (p * a) * i + torch.arange(npc * jx, npc * (jx + 1), dtype=torch.float).view(1, -1) for i in range(pc)]).unsqueeze(1) for jx in pos_map_mock[ix]]
...
def test_instpay():
    ix = 5
    exp_instpay_j = [torch.cat([nip * (p * a) * i + torch.arange(nip * jx, nip * (jx + 1), dtype=torch.float).view(1, -1) for i in range(ip)]).unsqueeze(1) for jx in instpay_map_mock[ix]]
...
def test_ccbal():
    ix = 8
    exp_ccbal_j = [torch.cat([nc * (p * a) * i + torch.arange(nc * jx, nc * (jx + 1), dtype=torch.float).view(1, -1) for i in range(c)]).unsqueeze(1) for jx in instpay_map_mock[ix]]
